...
class UILayout:
    def __init__(self, screen_width, screen_height):
        ...
        self.content_end_y = (
            screen_height * 0.8
        )  # 80% from top (where content area ends)
...
class BaseMenu(ABC):
    ...
    def draw_title(self, screen, title):
        ...
        title_rect = title_surface.get_rect(
            centerx=std_cfg.SCREEN_WIDTH / 2, y=self.layout.title_y
        )
    ...
    def draw_instructions(self, screen, instructions):
        ...
        for i, instruction in enumerate(instructions):
            text_surface = self.font.render(
                instruction, True, self.layout.colors["instructions"]
            )
            pos = self.layout.get_instruction_pos(
                i, text_surface.get_width(), total_instructions
            )
    ...
    def get_visible_item_count(self):
        ...
        return int(
            content_height / (self.layout.item_height + self.layout.item_padding)
        )
    ...
    def handle_scroll(self, event):
        ...
        if event.key == pygame.K_UP:
            ...
            if self.current_selection < total_items - 1:
                ...
                if self.current_selection >= self.scroll_offset + visible_items:
                    self.scroll_offset = min(
                        max_scroll, self.current_selection - visible_items + 1
                    )
            ...
            self.current_selection = min(
                self.scroll_offset + visible_items - 1, self.current_selection
            )
...
class MainMenu(BaseMenu):
    ...
    def draw(self, screen):
        ...
        for i, option in enumerate(self.options):
            ...
            # Draw text
            color = (
                self.layout.colors["selected_text"]
                if i == self.current_selection
                else self.layout.colors["text"]
            )
    ...
    def draw(self, screen):
        ...
        for i in range(start_idx, end_idx):
            ...
            text_rect = text_surface.get_rect(
                midleft=(item_rect.left + 10, text_pos[1])
            )
            screen.blit(text_surface, text_rect)
        ...
        if event.type == pygame.KEYDOWN:
            if event.key in (
                ...
                if self.songs:
                    return (
                        "START_GAME",
                        self.songs[self.current_selection]["filename"],
                    )
            elif event.key == pygame.K_ESCAPE:
...
class OptionsMenu(BaseMenu):
    ...
    def draw(self, screen):
        ...
        if self.waiting_for_key:
            ...
            ):  # Add scrolling instruction if scrollbar is present
                instructions.insert(1, "PgUp/PgDn: Scroll")
