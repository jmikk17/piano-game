...
class Song:
    ...
    def from_json(cls, filepath):
        ...
        for note_data in data["notes"]:
            song.add_note(note_data["bar"],note_data["slot"], note_data["note_type"], note_data["pitch"])
        return song
...
class MusicPlayer:
    ...
    def check_note_spawn(self):
        ...
        if current_time - self.last_update_time >= self.time_per_slot:
            ...
            self.spawn_notes()
        if self.song.b_path:
            if current_time - self.start_time >= self.play_b_time and not self.b_playing:
                self.b_playing = True
                self.b_track.play()


    def check_line_spawn(self):
        current_time = pygame.time.get_ticks() / 1000.0
        if self.line_spawned is False:
            if self.current_slot is std_cfg.SLOTS_PER_BAR-1 and current_time - self.last_update_time >= self.time_per_slot/2:
                self.spawn_line()
                self.line_spawned = True
        else:
            if self.current_slot == 0: self.line_spawned = False
    ...
    def check_note_hit(self, key):
        for note in self.active_notes:
            if note.hit or note.missed:
                ...
                # %7 or (+1)%7 here?????
            if (auxil.key_dictionary[note.pitch % 7] == key and 
